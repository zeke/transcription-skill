...
3. Transcribe the audio verbatim with `google/gemini-3.5-flash` on Replicate
...
The skill hardcodes `google/gemini-3.5-flash` as its transcription model.
That choice came from benchmarking it against `openai/whisper` (large-v3)
and three other Gemini variants on Replicate, transcribing the same
~7-minute narrated video with each and comparing prediction time and
transcript quality.

| Model | predict_time | total_time | Notes |
|---|---:|---:|---|
| **gemini-3.5-flash** | **18.6s** | **19.1s** | Fastest by a wide margin. Clean punctuation and paragraphing, accuracy on par with the pro models. |
| gemini-3-flash | 28.2s | 28.7s | Accurate, slightly terser phrasing than 3.5-flash. |
| gemini-3-pro | 35.2s | 35.6s | Natural casing and paragraphing, no accuracy gain over the flash variants to justify the extra time. |
| gemini-3.1-pro | 78.3s | 78.7s | Same accuracy as gemini-3-pro, ~4x slower than gemini-3.5-flash for no discernible quality gain. |
| openai/whisper (large-v3) | 93.2s | 116.9s | Slowest overall. Dense single-paragraph output, no paragraph breaks, lowercase proper nouns. |
| google/gemini-2.5-flash | n/a | n/a | Not testable — this model has no `audio` input on Replicate. |
